<body class="bg-[#111827] scroll-smooth">
    ...
    <x-app-layout>
        ...
        <div class="w-full p-5 flex flex-row-reverse">
            <form action="{{route('note-create')}}" method="GET">
                <button
                    class="mr-10 px-8 py-2 border-2 border-green-600 hover:bg-green-500 transition-all text-white rounded-md">
                    Agregar
                </button>
            </form>
            <div
                class="w-60  border-2 center rounded-md tex flex items-center content-center cursor-pointer pl-2 text-gray-500/50 border-gray-500/60 h-12 hover:text-white hover:border-gray-500 transition-all ">
                <?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                    fill="rgb(107 114 128 / 0.5)" width="24px" height="24px">
                    <path
        ...
        <div class="flex gap-10 m-10 content-center flex-wrap items-center object-center center">
            @foreach ($user->notes as $note)
            <form action="{{route('note-show', $note->id )}}" method="get">
                <label for="{{$note->id}}">
                    <div class="w-64 cursor-pointer h-48 bg-zinc-800 rounded-sm text-red flex flex-col">
                        <div class="w-full flex flex-row-reverse">
                            <label for="" class=" group p-4 elative m-2 rounded-sm cursor-pointer bg-zinc-600/60">
                                <?xml version="1.0" encoding="iso-8859-1"?>
                                <svg class="fill-white group-hover:fill-red-700 h-4 w-4 absolute translate-x-[-50%] translate-y-[-50%]" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
			 ...
			 M237.321,193.372h16.028v250.259h-16.028V193.372L237.321,193.372z M157.18,193.372h16.028v250.259H157.18V193.372z" />
                        ...
                        <div class=" rounded-sm  w-full h-[80%]">
                        </div>
                        <div
                            class="w-full h-[20%] bg-white  rounded-bl-sm rounded-br-sm  flex justify-center items-center text-center text-black">
                            <p>
                                {{$note->title}}
                            </p>
                        </div>
                ...
                <button type="submit" id="{{$note->id}}" class="hidden"></button>
            </form>
            <div class=" hidden w-screen h-screen bg-zinc-900/60 fixed">
            </div>
            @endforeach
        </div>
        <!------ //Note's seccion -------->
        ...
        <div class="w-full h-full flex flex-col items-center">
            <svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="400.67538" height="380.26155"
                viewBox="0 0 844.67538 595.26155" xmlns:xlink="http://www.w3.org/1999/xlink">
                ...
                <path
                    ...
                    transform="translate(-177.66231 -152.36922)" fill="#6c63ff" />
                <path
            ...
            <div class="text-center">
                <p class=" text-white"> Oh! it looks like u doesn't have any Note</p>
            </div>
            <div class="text-center mt-8">
                <form action="/note/create" method="GET">
                    @csrf
                    <button class="border-2 border-purple-800 hover:bg-purple-800 text-white py-3 rounded-sm px-10 ">
                        Let's
                        create one!</button>
                </form>
            </div>
